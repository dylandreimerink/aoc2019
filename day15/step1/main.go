		...
		outputCallback: func(out int) {
			if out == 0 {
				...
				if out == 2 {
					...
					spew.Dump("Done")
					//calc shortest path
					best := BreadthFirst(shipMap, droidX, droidY)
					fmt.Printf("Shortest path is: %d moves\n", best)
				}
			}
...
func BreadthFirst(shipMap [][]int, dx, dy int) int {
	depth := 1
	shipMap[25][25] = 5
	...
	for {
		...
		for y, row := range shipMap {
			for x, column := range row {
				if column == 5 {
					if shipMap[y+1][x] == 3 || shipMap[y-1][x] == 3 ||
						shipMap[y][x+1] == 3 || shipMap[y][x-1] == 3 {
						return depth
					}
