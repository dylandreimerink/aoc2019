...
func main() {
	...
	screen := &Screen{
		Entity: termloop.NewEntity(0, 0, screenWidth, screenHeight),
		Ticker: make(chan bool, 200),
	}

	game := termloop.NewGame()
	game.SetEndKey(termloop.KeyCtrlC)

	theLevel := termloop.NewBaseLevel(termloop.Cell{
		Bg: termloop.ColorBlack,
		Fg: termloop.ColorBlack,
		Ch: ' ',
	})

	game.Screen().SetLevel(theLevel)

	theLevel.AddEntity(screen)
	...
	comp := &IntComputer{
		...
		outputCallback: func(out int) {
			...
			if selectX == -1 && selectY == 0 {
				...
				if out < 0 || out > 4 {
					panic(fmt.Errorf("Unknown panel ID: %d", out))
				}
				...
				screen.SetCell(selectX, selectY, &termloop.Cell{
					Bg: termloop.ColorBlack,
...
// func renderScreen(game *termloop.Game, screen [][]int, score int) {
// 	fmt.Print("\033[2J")
