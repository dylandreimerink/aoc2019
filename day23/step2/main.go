...
func main() {
		...
		comp := &IntComputer{
			...
			outputCallback: func(out int) {
				...
				case 2:
					if outAddr == 255 {
						lastNatMutex.Lock()
						lastNatPacket = &packet{x: outX, y: out}
						lastNatMutex.Unlock()
						return
					}
	...
	for addr := 0; addr < 50; addr++ {
		go func(address int) {
			...
			fmt.Printf("Computer with addr %d has halted\n", address)
		}(addr)
	}
	...
	go func() {
		for {
			//Give the other routines a chance to reply
			time.Sleep(100 * time.Millisecond)

			idle := true
			for addr, queue := range networkMap {
				if len(queue) != 0 {
					fmt.Printf("Network not idle, addr: %d has %d packets left\n", addr, len(queue))
					idle = false
				}
			}
			if idle && lastNatPacket != nil {
				lastNatMutex.Lock()
				fmt.Printf("From NAT to 0 - x: %d, y: %d\n", lastNatPacket.x, lastNatPacket.y)
				networkMap[0] <- *lastNatPacket
				if lastNatPacket.y == lastYSent {
					fmt.Printf("Repeated Y from NAT to 0: %d\n", lastYSent)
					os.Exit(0)
				}
				lastYSent = lastNatPacket.y
				lastNatMutex.Unlock()
			}
		}
	}()
