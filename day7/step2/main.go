			...
			for p2 := 5; p2 <= 9; p2++ {
				...
				for p3 := 5; p3 <= 9; p3++ {
					...
					for p4 := 5; p4 <= 9; p4++ {
						...
						var inputCountA, inputCountB, inputCountC, inputCountD, inputCountE int

						chanA := make(chan int, 20)
						chanB := make(chan int, 20)
						chanC := make(chan int, 20)
						chanD := make(chan int, 20)
						chanE := make(chan int, 20)
						...
						compA := IntComputer{
							...
							inputCallback: func() string {
								if inputCountA == 0 {
									inputCountA++
									return strconv.Itoa(p0)
								}

								return strconv.Itoa(<-chanA)
							},
						...
						compD := IntComputer{
							...
							inputCallback: func() string {
								if inputCountD == 0 {
									inputCountD++
									return strconv.Itoa(p3)
								}

								return strconv.Itoa(<-chanD)
							},
							outputCallback: func(output int) {
								chanE <- output
							},
						...
						compE := IntComputer{
							...
							inputCallback: func() string {
								if inputCountE == 0 {
									inputCountE++
									return strconv.Itoa(p4)
								}

								return strconv.Itoa(<-chanE)
							},
							outputCallback: func(output int) {
								chanA <- output
								if output > maxOutput {
